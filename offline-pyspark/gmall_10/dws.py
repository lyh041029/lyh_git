...
print("=== 数据质量检查 ===")
# 检查DWD层数据总量
total_count = spark.sql("SELECT COUNT(*) as count FROM gmall_10.dwd_user_behavior_log WHERE dt = '20250808'").collect()[0]['count']
...
spark.sql("""
    ...
    FROM gmall_10.dwd_user_behavior_log 
    WHERE dt = '20250808' 
    GROUP BY event_type 
    ORDER BY count DESC
""").show()
...
spark.sql("""
    ...
    FROM gmall_10.dwd_user_behavior_log 
    WHERE dt = '20250808' AND event_type = 'search' AND search_rank IS NOT NULL
""").show()
...
spark.sql("""
    ...
    FROM (
        ...
        FROM gmall_10.dwd_user_behavior_log 
        WHERE dt = '20250808' AND store_id IS NOT NULL AND item_id IS NOT NULL
...
# ====================== DWS层页面流量分析日汇总表 ======================
# 1. 创建HDFS目录
create_hdfs_dir("/warehouse/gmall_10/dws/dws_page_traffic_analysis_d")

# 2. 删除旧表
spark.sql("DROP TABLE IF EXISTS gmall_10.dws_page_traffic_analysis_d")

# 3. 创建外部表
spark.sql("""
CREATE EXTERNAL TABLE gmall_10.dws_page_traffic_analysis_d (
...
LOCATION '/warehouse/gmall_10/dws/dws_page_traffic_analysis_d'
TBLPROPERTIES ('orc.compress' = 'snappy');
""")

# 4. 数据处理
page_traffic_df = spark.table("gmall_10.dwd_user_behavior_log").filter(
    (F.col("dt") == "20250808") &
...
page_traffic_df.write.mode("overwrite") \
    .partitionBy("dt") \
    .orc("/warehouse/gmall_10/dws/dws_page_traffic_analysis_d")
...
# ====================== DWS层页面点击分布日汇总表 ======================
# 1. 创建HDFS目录
create_hdfs_dir("/warehouse/gmall_10/dws/dws_page_click_distribution_d")

# 2. 删除旧表
spark.sql("DROP TABLE IF EXISTS gmall_10.dws_page_click_distribution_d")

# 3. 创建外部表
spark.sql("""
CREATE EXTERNAL TABLE gmall_10.dws_page_click_distribution_d (
    `store_id`             STRING COMMENT '店铺ID',
    `page_type`            STRING COMMENT '页面类型(home-首页,item_list-商品列表,item_detail-商品详情,search_result-搜索结果)',
...
PARTITIONED BY (dt string COMMENT '统计日期')STORED AS ORC
LOCATION '/warehouse/gmall_10/dws/dws_page_click_distribution_d'
TBLPROPERTIES ('orc.compress' = 'snappy');
""")

# 4. 数据处理 - 修复后的代码
click_distribution_df = spark.table("gmall_10.dwd_user_behavior_log").filter(
    (F.col("dt") == "20250808") &
    (F.col("store_id").isNotNull()) &
    (F.col("page_type").isNotNull())
).groupBy("store_id", "page_type").agg(
...
click_distribution_df.write.mode("overwrite") \
    .partitionBy("dt") \
    .orc("/warehouse/gmall_10/dws/dws_page_click_distribution_d")
...
# ====================== DWS层页面引导效果日汇总表 ======================
# 1. 创建HDFS目录
create_hdfs_dir("/warehouse/gmall_10/dws/dws_page_guidance_effect_d")

# 2. 删除旧表
spark.sql("DROP TABLE IF EXISTS gmall_10.dws_page_guidance_effect_d")

# 3. 创建外部表
spark.sql("""
CREATE EXTERNAL TABLE gmall_10.dws_page_guidance_effect_d (
    `store_id`                 STRING COMMENT '店铺ID',
    `page_type`                STRING COMMENT '页面类型(home-首页,item_list-商品列表,item_detail-商品详情,search_result-搜索结果)',
    `page_views`               BIGINT COMMENT '页面浏览量',
...
PARTITIONED BY (dt string COMMENT '统计日期')STORED AS ORC
LOCATION '/warehouse/gmall_10/dws/dws_page_guidance_effect_d'
TBLPROPERTIES ('orc.compress' = 'snappy');
""")

# 4. 数据处理 - 修复后的代码
guidance_effect_df = spark.table("gmall_10.dwd_user_behavior_log").filter(
    (F.col("dt") == "20250808") &
    (F.col("store_id").isNotNull()) &
    (F.col("page_type").isNotNull())
).groupBy("store_id", "page_type").agg(
...
guidance_effect_df.write.mode("overwrite") \
    .partitionBy("dt") \
    .orc("/warehouse/gmall_10/dws/dws_page_guidance_effect_d")
...
SELECT
    ...
    (SELECT COUNT(*) FROM gmall_10.dws_page_traffic_analysis_d WHERE dt = '20250808') as traffic_count,
    (SELECT COUNT(*) FROM gmall_10.dws_page_click_distribution_d WHERE dt = '20250808') as click_count,
    (SELECT COUNT(*) FROM gmall_10.dws_page_guidance_effect_d WHERE dt = '20250808') as guidance_count
""").show()

# 查看部分结果示例
spark.sql("SELECT * FROM gmall_10.dws_page_traffic_analysis_d WHERE dt = '20250808' LIMIT 5").show()
spark.sql("SELECT * FROM gmall_10.dws_page_click_distribution_d WHERE dt = '20250808' LIMIT 5").show()
spark.sql("SELECT * FROM gmall_10.dws_page_guidance_effect_d WHERE dt = '20250808' LIMIT 5").show()
